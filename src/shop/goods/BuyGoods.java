...
public class BuyGoods extends HttpServlet
{
        ...
        throws ServletException, IOException
    {
        ...
        if (car.size()!=0)
        {
            ...
            //按号码 - 修改库存
            sqlOrder = "insert into orderform(username,commodity_name,commodity_price,sum,id) values(?,?,?,?,?)";
            //插入商品数据到订单表 1用户名 2货物名-购物车容器读取 3价格-购物车容器读取 4-总数
            //将商品行集数据遍历到数组中
                            ...
                            for (int j = 0,n=goods.length; j < n; j++)
                            {
                                ...
                                switch (j)
                                {
                                    ...
                                    case 3:
                                        ...
                                        break;
                                    case 4:
                                            Double commodity_balance = -1.0;
                                            System.out.println(Double.parseDouble(goods[4]));
                                            commodity_balance = Double.parseDouble(goods[4])-1;//目前是默认每次修改一个
                                            System.out.println(commodity_balance);
                                            if (commodity_balance >= 0)
                                            {
                                                pstmtCommodity.setDouble(1,commodity_balance);
                                            }else 
                                                {
                                                    String failNumber = "商品不足";
